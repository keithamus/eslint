...
module.exports = {
    ...
    execute: function(context) {
        ...
        */
        function reportNoBeginningSpace(node, token) {
            context.report({
                node: node,
                loc: token.loc.start,
                message: this.metadata.errors.extraSpaceAfter,
                data: {
                    token: token.value
                },
                fix: function(fixer) {
        ...
        */
        function reportNoEndingSpace(node, token) {
            context.report({
                node: node,
                loc: token.loc.start,
                message: this.metadata.errors.extraSpaceBefore,
                data: {
                    token: token.value
                },
                fix: function(fixer) {
        ...
        */
        function reportRequiredBeginningSpace(node, token) {
            context.report({
                node: node,
                loc: token.loc.start,
                message: this.metadata.errors.spaceAfter,
                data: {
                    token: token.value
                },
                fix: function(fixer) {
        ...
        */
        function reportRequiredEndingSpace(node, token) {
            context.report({
                node: node,
                loc: token.loc.start,
                message: this.metadata.errors.spaceBefore,
                data: {
                    token: token.value
                },
                fix: function(fixer) {
        ...
        */
        function validateArraySpacing(node) {
            ...
            if (astUtils.isTokenOnSameLine(first, second)) {
                if (openingBracketMustBeSpaced && !sourceCode.isSpaceBetweenTokens(first, second)) {
                    reportRequiredBeginningSpace.call(this, node, first);
                }
                if (!openingBracketMustBeSpaced && sourceCode.isSpaceBetweenTokens(first, second)) {
                    reportNoBeginningSpace.call(this, node, first);
                }
            }

            if (first !== penultimate && astUtils.isTokenOnSameLine(penultimate, last)) {
                if (closingBracketMustBeSpaced && !sourceCode.isSpaceBetweenTokens(penultimate, last)) {
                    reportRequiredEndingSpace.call(this, node, last);
                }
                if (!closingBracketMustBeSpaced && sourceCode.isSpaceBetweenTokens(penultimate, last)) {
                    reportNoEndingSpace.call(this, node, last);
                }
            }
        }
        ...
        return {
            ArrayPattern: validateArraySpacing.bind(this),
            ArrayExpression: validateArraySpacing.bind(this)
        };
