...
function createOptions(options, schema, meta) {
    ...
    for (var i = 0, l = meta.schema.length; i < l; i++) {
        if (meta.schema[i].type && meta.schema[i].type === "object") {
            Object.keys(meta.schema[i].properties).forEach(function(key) {
                result[key] = options[i] && typeof options[i][key] !== "undefined" ? options[i][key] : meta.schema[i].properties[key].default;
            });
        }
        if (meta.schema[i].enum) {
            var value = options[i];
            meta.schema[i].enum.forEach(function(enumValue) {
